"""
Провайдер cookies через внешнее API (spfa.ru)
"""
import json
import time
...
class ExternalApiCookiesProvider(CookiesProvider):
    """Получает cookies через внешнее API (spfa.ru)"""
    ...
    def __init__(self, api_key: str, storage_path: str | Path = "storage/cookies_external.json"):
        ...
        self.WAIT_FIRST_FOR_UNBLOCK = 5  # 5 сек
    ...
    def _get_new_cookies(self) -> Dict[str, str]:
        """Получить новые cookies через API"""
        try:
    ...
    def _try_unblock(self):
        """Попытка разблокировать cookies"""
        if not self.last_id:
            return
